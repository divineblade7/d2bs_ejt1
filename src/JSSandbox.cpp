...
JSAPI_PROP(sandbox_getProperty) {
    ...
    if (JSVAL_IS_INT(ID)) {
        ...
        if (box && JS_LookupProperty(box->context, box->innerObj, name, vp.address()))
            return JS_TRUE;
        if (JSVAL_IS_VOID(vp.get()) &&
            JS_LookupProperty(cx, obj, name, vp.address()))
            return JS_TRUE;
    } else if (JSVAL_IS_STRING(ID)) {
        char* name = JS_EncodeStringToUTF8(cx, JSVAL_TO_STRING(ID));
        vp.set(JSVAL_VOID);
        if (box && (JS_LookupProperty(box->context, box->innerObj, name,
                                      vp.address()))) {
            JS_free(cx, name);
            return JS_TRUE;
        }
...
JSAPI_STRICT_PROP(sandbox_setProperty) {
    ...
    if (JSVAL_IS_INT(ID)) {
        ...
        char* name = JS_EncodeStringToUTF8(cx, JSVAL_TO_STRING(ID));
        if (box &&
            JS_SetProperty(box->context, box->innerObj, name, vp.address())) {
            JS_free(cx, name);
            return JS_TRUE;
        }
        JS_free(cx, name);
