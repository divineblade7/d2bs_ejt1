...
void LevelMap::AddCollisionMap(Room1* pRoom1) {
  ...
  int x = map->dwPosGameX - posX, y = map->dwPosGameY - posY, _height = map->dwPosGameX + map->dwSizeGameX - posX,
      _width = map->dwPosGameY + map->dwSizeGameY - posY;
  ...
  for (int i = y; i < _width && p != map->pMapEnd; i++) {
    for (int j = x; j < _height && p != map->pMapEnd; j++, p++) {
      if ((*p & LevelMap::ClosedDoor) == LevelMap::ClosedDoor || (*p & LevelMap::Player) == LevelMap::Player) {
      ...
      } else if (((*p & LevelMap::FriendlyNPC) == LevelMap::FriendlyNPC ||
        ...
        SetCollisionData(j, i, 0);
      } else if ((*p & LevelMap::AlternateTile) == LevelMap::AlternateTile &&
                 (*p & LevelMap::BlockWalk) != LevelMap::BlockWalk && (*p & LevelMap::Wall) != LevelMap::Wall) {
        SetCollisionData(j, i, 0);
      } else {
  ...
  for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next) {
    ...
    for (DWORD i = 0; i < room->dwRoomsNear; i++) {
      ...
      if (overlappingX < 0 || overlappingY < 0) {
        // Print("ÿc1d2bsÿc3LevelMap::GetExitsÿc0 adjecent room is out of reach - it's not even touching local room in
        // corner!");
        continue;
      }
      if (overlappingX > 0 && overlappingY > 0) {
...
bool LevelMap::SpaceIsWalkable(const Point& point, bool abs) const {
  ...
  return !(SpaceHasFlag(LevelMap::Avoid, point, abs) || SpaceHasFlag(LevelMap::BlockWalk, point, abs) ||
...
bool LevelMap::SpaceIsWalkableForExit(const Point& point, bool abs) const {
  ...
  return !(!IsValidPoint(point, abs) || SpaceHasFlag(LevelMap::Avoid, point, abs) ||
...
bool LevelMap::ValueIsWalkable(const WORD* value) const {
  return !(ValueHasFlag(LevelMap::BlockWalk, value) || ValueHasFlag(LevelMap::BlockPlayer, value) ||
