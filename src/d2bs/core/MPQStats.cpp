...
MPQTable BaseStatTable[] = {
    ...
    {0x96C9F0, 0x96C9FC, lvltypesTable, "lvltypes", ARRAYSIZE(lvltypesTable),
     0xFFFF},  // v - Fixed to fully dump the same as d2jsp - TechnoHunter
...
bool FillBaseStat(int table, int row, int column, void* result, size_t size) {
  ...
  switch (pTable[column].eFieldType) {
    case FIELDTYPE_DATA_ASCII:
      ...
      memcpy_s(result, pTable[column].dwFieldLength, (BYTE*)(dwRetValue + pTable[column].dwFieldOffset),
...
DWORD FillBaseStat(JSContext* cx, JS::Value* argv, int table, int row, int column, const char* szTable,
                   const char* szStat) {
  if (szTable) {
    table = -1;
    for (int i = 0; BaseStatTable[i].pTable != NULL; i++)
  ...
  if (column > BaseStatTable[table].wTableSize) return FALSE;
