...
BOOL OOG_CreateCharacter(const wchar_t* szCharacter, int type, [[maybe_unused]] bool hardcore,
...
BOOL OOG_SelectGateway(const wchar_t* szGateway, [[maybe_unused]] size_t strSize) {
  if (ClientState() != ClientStateMenu) return FALSE;
...
bool OOG_JoinGame(const wchar_t* name, const wchar_t* pass) {
  ...
  if (!(OOG_GetLocation() == OOG_LOBBY || OOG_GetLocation() == OOG_CHAT || OOG_GetLocation() == OOG_JOIN)) return FALSE;
