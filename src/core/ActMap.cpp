...
bool ActMap::isPointInRoom(const Room2* room, const Point& pt) const {
  ...
  return (nX >= room->dwPosX * 5 && nY >= room->dwPosY * 5 && nX < (room->dwPosX * 5 + room->dwSizeX * 5) &&
...
bool ActMap::isPointInLevel(const Level* lvl, const Point& pt) const {
  DWORD x = pt.first;
  DWORD y = pt.second;
  return (x >= lvl->dwPosX * 5 && y >= lvl->dwPosY * 5 && x < (lvl->dwPosX * 5 + lvl->dwSizeX * 5) &&
...
void ActMap::FindRoomLinkageExits(ExitArray& exits) const {
  ...
  for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next) {
    ...
    for (DWORD i = 0; i < room->dwRoomsNear; i++) {
      ...
      if (overlappingX < 0 || overlappingY < 0) {
        // Print("пїЅc1d2bsпїЅc3ActMap::GetExitsпїЅc0 adjecent room is out of reach - it's not even touching local room
        // in corner!");
        continue;
      }
      if (overlappingX > 0 && overlappingY > 0) {
        // Print("пїЅc1d2bsпїЅc3ActMap::GetExitsпїЅc0 WTF local and adjecent rooms are overlapping (they share some
        // points)!!!");
        continue;
      }
      ...
      for (j = 0; j < edgeSize; j++) {
        ...
        if (false == isPointWalkable || j + 1 == edgeSize) {
          if (spaces > 0) {
            ...
            currentPoint = Point(lastWalkablePoint.first - edgeDirection.first * spaces / 2,
                                 lastWalkablePoint.second - edgeDirection.second * spaces / 2);
            exitMap.insert(
                make_pair(rooms[i]->pLevel->dwLevelNo, make_pair(centerEdgePoint, make_pair(currentPoint, spaces))));
          }
          spaces = 0;
        }
      }
  ...
  do {
    ...
    tmpDistance = GetDistance(it->second.first.first, it->second.first.second, it->second.second.first.first,
...
bool ActMap::EdgeIsWalkable(const Point& edgePoint, const Point& offsetPoint, bool abs) const {
  ...
  for (k = distanceLocal; k <= distanceAdjecent; k++) {
    ...
    {
      const Point currentPoint = Point(edgePoint.first + offsetPoint.first * (distanceLocal - k),
...
bool ActMap::SpaceIsWalkable(const Point& point, bool abs) const {
  ...
  return !(SpaceHasFlag(ActMap::Avoid, point, abs) || SpaceHasFlag(ActMap::BlockWalk, point, abs) ||
...
void ActMap::DumpLevel(const char* file) const {
  ...
  for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next) {
    for (PresetUnit* preset = room->pPreset; preset; preset = preset->pPresetNext) {
      ...
      fprintf(f, "Preset %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, _1 %d, _3 %d, room (%d, %d) %s\n",
              i++, loc.first, loc.second, rel.first, rel.second, preset->dwType, preset->dwTxtFileNo, preset->_1,
              preset->_3, room->dwPosX, room->dwPosY, wName);
    }
  }
  ...
  for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next) {
    ...
    for (UnitAny* pUnit = room->pRoom1->pUnitFirst; pUnit; pUnit = pUnit->pRoomNext) {
      ...
      fprintf(f, "Unit %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, %s, %d, %d\n",
              ...
              i++, loc.first, loc.second, rel.first, rel.second, pUnit->dwType, pUnit->dwTxtFileNo, tmp, pUnit->wX,
              pUnit->wY);
    }
  }
  RoomList::iterator start = added.begin(), last = added.end();
  ...
  for (ExitArray::iterator it = exits.begin(); it != exits.end(); it++) {
    ...
    fprintf(f, "Exit %d: at (%d, %d, relative: %d, %d) of type %s linkage to area %s (id: %d, tile id: %d)\n", i++,
            it->Position.first, it->Position.second, rel.first, rel.second, it->Type == Linkage ? "Area" : "Tile",
            GetLevelIdName(it->Target), it->Target, it->TileId);
