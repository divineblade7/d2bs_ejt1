  ...
  */
  ...
  void Reduce(PointList const& in, PointList& out, bool) {
    ...
    while (it != end) {
        ...
        if (lineLength > 5 && ((currentStep != lineStartStep && currentStep != secondStep) || differentStepCount > 1 ||
    lineLength > 20)) { out.push_back(previousPoint); lineStartStep = Point(currentStep); secondStep = lineStartStep;
            lineLength = 1;
            differentStepCount = 0;
        }
    }
  ...
  int GetPenalty(Point const& pt, bool abs) {
    ...
    return 0;
    // return map->SpaceHasFlagWide(ActMap::BlockWalk, pt, abs) ? 50 : (data & ActMap::Object == ActMap::Object) ? 60 :
    // (map->SpaceHasFlag(ActMap::ClosedDoor, pt, abs) ? 80 : 0));
  }
  ...
  void MutatePoint(Point& pt, bool absv) {
    ...
    for (int i = -2; i <= 2; i++) {
      for (int j = -2; j <= 2; j++) {
        ...
        if (!checkFlag(area[3 + i][3 + j] | area[3 + i + 1][3 + j] | area[3 + i - 1][3 + j] | area[3 + i][3 + j + 1] |
                       area[3 + i][3 + j - 1])) {
          pt.first += i;
          pt.second += j;
          mutated = true;
